...
const specialists: Array<{ name: AgentName; instructions: string }> = [
  ...
  { name:'Web Architect', instructions:`You are a principal web architect and senior full-stack engineer. Evaluate route architecture, consistency across pages, performance signals, technical quality, broken or fragile behavior, forms and interaction coverage, production readiness, maintainability, security implications, and whether the site behaves like a coherent production system. Distinguish verified evidence from reasonable architectural inference.` },
  { name:'QA Analyst', instructions:`You are a senior QA and accessibility lead. Evaluate the supplied site-wide evidence for broken routes, link failures, inconsistent page states, missing labels or semantics, incomplete controls, accessibility risks, error-state concerns, interaction coverage gaps, and regression risks. Prioritize issues that would materially affect real users.` },
  { name:'Vibe-Code Reviewer', instructions:`You are an expert reviewer of AI-assisted and vibe-coded web applications with strong SEO/AEO and product-quality expertise. Look for generic patterns, repetitive layouts, excessive spacing, oversized headings, weak content hierarchy, generic typography, unnecessary cards/gradients, inconsistent patterns, thin page content, weak metadata, production shortcuts, and signs that the experience needs human-level refinement.` },
];

function compactEvidence(audit: AuditResult) {
  const pages = audit.pages.map((page) => ({ path:safePath(page.url), status:page.status, title:page.title, sections:page.sections??0, links:page.links??0, buttons:page.buttons??0, forms:page.forms??0, images:page.images??0, headings:page.headings??0 }));
  const deterministicFindings = audit.findings.slice(0,120).map((finding) => ({ priority:finding.priority, category:finding.category, title:finding.title, page:finding.page, detail:finding.detail, evidence:finding.evidence }));
  return JSON.stringify({ site:audit.url, summary:audit.summary, coverage:audit.coverage, scores:audit.scores, pages, deterministicFindings });
}
...
function normalizeFinding(raw: AiFinding, agent: AgentName, siteUrl: string): Finding | null {
  ...
  const finding={priority,category,title,detail,recommendation,page:typeof raw.page==='string'?raw.page.trim().slice(0,500):undefined,evidence:typeof raw.evidence==='string'?raw.evidence.trim().slice(0,1200):undefined,agent} satisfies Finding;
...
async function runSpecialist(agentName: AgentName, instructions: string, evidence: string, siteUrl: string) {
  ...
  return { findings, positives:(payload.positives??[]).filter((item):item is string=>typeof item==='string').map(item=>item.trim()).filter(Boolean).slice(0,5), assessment:typeof payload.overallAssessment==='string'?payload.overallAssessment.trim().slice(0,1200):'' };
}
...
export async function runAiExpertReview(audit: AuditResult): Promise<AiReviewResult> {
  ...
  if(!hasGatewayContext) {
    return {
      ...
      limitation:'The deterministic site-wide audit completed. Model-driven expert review was skipped because AI Gateway authentication is not available in this environment.',
    };
  }
  ...
  try {
    ...
    return { findings, positives:[...new Set(results.flatMap(result=>result.positives))].slice(0,12), assessments:results.map(result=>result.assessment).filter(Boolean), aiEnhanced:true };
  } catch(error) {
    const reason=error instanceof Error?error.message:'The AI expert layer could not be reached.';
    return { findings:[], positives:[], assessments:[], aiEnhanced:false, limitation:`The deterministic site-wide audit completed, but the model-driven expert review was unavailable for this run. ${reason}` };
  }
}
